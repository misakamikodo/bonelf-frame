...
public class NettyWebSocketHandler extends SimpleChannelInboundHandler<WebSocketFrame> implements BnfWsHandler {
	...
	private void handleHandleCompleteEvt(ChannelHandlerContext ctx, Object evt) {
		...
		if (StringUtils.hasText(userId)) {
			...
			if (flag) {
				...
				ctx.channel().attr(NettyWebsocketMap.USER_ID_CHANNEL_KEY).set(userId);
				nettyWebsocketMap.getSocketSessionMap().put(userId, ctx.channel());
	...
	private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
		if (frame instanceof TextWebSocketFrame) {
			...
			handleTextMessage(ctx.channel().attr(NettyWebsocketMap.USER_ID_CHANNEL_KEY).get(), (TextWebSocketFrame)frame);
			return;
		}
